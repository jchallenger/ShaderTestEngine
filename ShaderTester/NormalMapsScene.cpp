...
NormalMapsScene::NormalMapsScene(void) : Scene()
{
}


NormalMapsScene::~NormalMapsScene(void)
{
}

bool NormalMapsScene::init()
{
    Scene::init();
    gui = new GUI();
    gui->setPos(300,400);
    ...
    OSpaceNormals = new Shader("Resources/Shaders/ONormal");
    OSpaceNormals->setName("ObjectSpaceNormals");

    TSpaceNormals = new Shader("Resources/Shaders/TNormal");
    TSpaceNormals->setName("TangentSpaceNormals");
    ...
    temp->normalMap = TextureUtils::loadTexture("Resources/Images/Sphere_T_Norm.png");
    ...
    temp->normalMap = TextureUtils::loadTexture("Resources/Images/Corner_T_Norm.png");
    temp->setStartPosition(0,0,0);
    temp->setPosition(0,0,0);
    temp->textureType = DIF_MAP;
    temp->setScene(this);
    ...
    temp->normalMap = TextureUtils::loadTexture("Resources/Images/AlienShip_T_Norm.png");
...
void NormalMapsScene::update(float delta){
    Scene::update(delta);
    //*****************************************************************************
    {
    // Rotate camera
...
void NormalMapsScene::draw(){

    for(int L = 0; L < Light::numOfLights; L++)
        light[L]->render();

    if(WindowInfo::getInstance()->isFullScreen()){
        glViewport(0, 0, WindowInfo::getInstance()->getWidth(), 600);
...
void NormalMapsScene::drawBorders(){
...
void NormalMapsScene::drawOverlay(){
    glUseProgram(0);
